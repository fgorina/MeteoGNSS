...
// static uint8_t BMI270_ADDR = BMI2_I2C_SEC_ADDR;
// static uint8_t aux_bmm150_dev_addr = BMM150_DEFAULT_I2C_ADDRESS;
...
namespace BMI270 {
    ...
    int BMI270::init(i2c_port_t i2c_port, uint8_t bmiAddr)
    {
        _i2c_port = i2c_port;
        ...
        if (bmm1.chip_id == BMM150_CHIP_ID) {
            printf("id ok\n");
            bmm150_init_status = true;
        }
        else {
            printf("error id\n");
            bmm150_init_status = false;
        }
    ...
    bool BMI270::bmm150isEnabled(){
        return bmm150_init_status;
    }
    ...
    int8_t BMI270::configure_sensor(struct bmm150_dev *dev)
    {
        ...
        if (rslt == BMM150_OK)
        {
            ...
            */
            ...
            // settings.data_rate = BMM150_DATA_RATE_02HZ;
    ...
    int8_t BMI270::bmi2_i2c_read(uint8_t reg_addr, uint8_t *reg_data, uint32_t len, void *intf_ptr)
    {
        ...
        Wire.beginTransmission(_dev_info->dev_addr);
        Wire.write(reg_addr);
        if (Wire.endTransmission(false) != 0) {
            ESP_LOGE(TAG, "R %02X %02X endTransmission failed", _dev_info->dev_addr, reg_addr);
            return -1;
        }
        Wire.requestFrom((uint8_t)_dev_info->dev_addr, (uint8_t)len);
    ...
    int8_t BMI270::bmi2_i2c_write(uint8_t reg_addr, const uint8_t *reg_data, uint32_t len, void *intf_ptr)
    {
        ...
        Wire.beginTransmission(_dev_info->dev_addr);
        Wire.write(reg_addr);
        Wire.write(reg_data, (int)len);
        if (Wire.endTransmission() != 0) {
            ESP_LOGE(TAG, "W %02X %02X endTransmission failed", _dev_info->dev_addr, reg_addr);
            return -1;
        }
